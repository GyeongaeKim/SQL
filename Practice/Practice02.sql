...
--예) 2014년 07월 10일
select rpad(to_char(max(hire_date), 'YYYY'),6,'년')||' '||
       rpad(to_char(max(hire_date), 'MM'),4,'월')||' '||
       rpad(to_char(max(hire_date), 'DD'),4,'일') 마지막신입사원
from employees;
...
--입사일이 02/12/31일 이전이면 '창립맴버, 03년은 '03년입사’, 
--04년은 ‘04년입사’ 이후입사자는 ‘상장이후입사’ optDate 컬럼의 데이터로 출력하세요.
